...
CKEDITOR.dialog.add( 'link', function( editor ) {
	...
	return {
		...
		onShow: function() {
            ...
            var id_file = document.getById( 'id_file' );
            var oldVal = id_file.getValue();

            setInterval(function () {
                if (oldVal != id_file.getValue()) {
                    oldVal = id_file.getValue();
                    var url = dialog.getContentElement("info", "url");
        			url.setValue("{{ " + id_file.getValue() + "|file_url }}");
                }
            }, 1000);

            var id_file_clear = document.getById( 'id_file_clear' );

            id_file_clear.on('click', function () {
                id_file.setValue("");
                id_file.removeAttribute("value");
                id_file_thumbnail_img = document.getById( 'id_file_thumbnail_img' );
                id_file_thumbnail_img.setAttribute("src", "/static/filer/icons/nofile_48x48.png");
                id_file_description_txt = document.getById( 'id_file_description_txt' );
                id_file_description_txt.setHtml("");
                id_file_clear = document.getById( 'id_file_clear' );
                id_file_clear.hide();
            });

			if ( this.insertMode )
				id_file_clear.fire('click');
